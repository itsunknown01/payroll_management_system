...
import { deleteDeduction } from "@/actions/deduction/delete-deduction";
...
const DeleteDeductionModal = () => {
  ...
  const isModalOpen = isOpen && type == "deleteDeduction";
  const { deduction } = data;

  const handleConfirm = () => {
    startTransition(() => {
      deleteDeduction(deduction?.id).then((data) => {
  ...
  return (
    <Modal isOpen={isModalOpen} onClose={onClose} title="Delete Deduction">
